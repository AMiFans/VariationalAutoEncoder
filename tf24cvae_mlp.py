...
class VariationalAutoEncoder(tf.keras.Model):
    ...
    def call(self, inputs):
        x, y = inputs
        s = tf.concat([x, y], axis=-1)
        z_mean, z_log_var, z = self.encoder(s)
        z = tf.concat([z, y], axis=-1)
        reconstructed = self.decoder(z)
        # Add KL divergence regularization loss.
        kl_loss = - 0.5 * tf.reduce_mean(
            z_log_var - tf.square(z_mean) - tf.exp(z_log_var) + 1)
        self.add_loss(0.5*kl_loss)
...
y_train, y_test = y_train.reshape(-1, 10).astype('float32'), y_test.reshape(-1, 10).astype('float32')
...
arr = sorted([i for i in range(10)]*10)
y = np.eye(10)[arr]
test_input = tf.concat([random_vector_for_generation, y], axis=-1)
...
def generate_and_save_images(model, epoch, test_input):
    ...
    for i in range(predictions.shape[0]):
        ...
        plt.axis('off')
    plt.savefig('./save_images/cvae_mlp_image_at_epoch_{:04d}.png'.format(epoch))
    #plt.show()
...
for epoch in range(epochs):
    ...
    for step, batch_train in enumerate(train_dataset):
        x__train, y_train = batch_train
        with tf.GradientTape() as tape:
            reconstructed = vae((x__train, y_train))
            # 计算重构损失
            loss = mse_loss_fn(x__train, reconstructed)
...
vae.save_weights('save_weights/cvae_mlp_weights')
